...
class GLMRequestBuilder:
    ...
    def build_request(
        ...
        # 构建基础请求
        request = {
            "model": self.model_name,  # 统一使用 glm-4.6
            "messages": messages,
...
class GLMModelManager:
    ...
    def get_request_builder(self, model_name: str = "glm-4.6") -> GLMRequestBuilder:
        """获取请求构建器"""
        return GLMRequestBuilder(model_name)

    def get_all_models(self) -> List[Dict[str, Any]]:
        """获取所有GLM模型信息（统一使用 glm-4.6，显示不同模式）"""
        modes = GLMModelConfig.get_all_modes()
        models = []
        for mode in modes:
            models.append(
                {
                    "id": f"glm-4.6-{mode['id']}",
                    "name": mode["name"],
                    "description": mode["description"],
                    "model_name": "glm-4.6",  # 统一使用 glm-4.6
                    "mode": mode["id"],  # 思考模式

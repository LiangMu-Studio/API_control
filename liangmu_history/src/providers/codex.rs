...
impl CodexProvider {
    ...
    fn get_cwd_fast(file_path: &Path) -> Option<String> {
        ...
        for line in reader.lines() {
            ...
            if line.contains("\"cwd\"") {
                ...
                if let Some(cwd) = data.get("payload").and_then(|p| p.get("cwd")).and_then(|v| v.as_str()) {
    ...
    fn parse_codex_message(data: &Value) -> Option<Message> {
        ...
        match msg_type {
            ...
            "event_msg" => {
                ...
                let message_text = payload.get("message").and_then(|v| v.as_str()).unwrap_or("");
    ...
    fn parse_session_file(&self, file_path: &Path) -> Option<Session> {
        ...
        for line in reader.lines() {
            ...
            // 提取 cwd
            if cwd.is_none() {
                if let Some(c) = data.get("payload").and_then(|p| p.get("cwd")).and_then(|v| v.as_str()) {
                    cwd = Some(c.to_string());
                }
            }

            let ts = data.get("timestamp").and_then(|v| v.as_str()).map(String::from);
    ...
    fn parse_session_info(&self, file_path: &Path) -> Option<SessionInfo> {
        ...
        for line in reader.lines() {
            ...
            match msg_type {
                Some("response_item") => {
                    if data.get("payload").and_then(|p| p.get("type")).and_then(|v| v.as_str()) == Some("message") {
                        msg_count += 1;
                    }
                }
                Some("event_msg") => {
                    let event_type = data.get("payload").and_then(|p| p.get("type")).and_then(|v| v.as_str());
...
impl CliHistoryProvider for CodexProvider {
    ...
    fn load_project(&self, cwd_path: &str) -> Vec<SessionInfo> {
        ...
        let cwd_normalized = cwd_path.replace('\\', "/").to_lowercase();
        ...
        let mut sessions: Vec<SessionInfo> = files
            ...
            .filter_map(|file_path| {
                ...
                if cwd.replace('\\', "/").to_lowercase() != cwd_normalized {
                    return None;
                }
    ...
    fn load_session_paginated(
        ...
        let last: Vec<Message> = rounds[total_turns - last_turns..].iter().flatten().cloned().collect();
    ...
    fn delete_session(&self, file_path: &str) -> Result<(), String> {
        ...
        let mut manifest: crate::types::TrashManifest = if manifest_path.exists() {
            ...
            serde_json::from_str(&content).unwrap_or(crate::types::TrashManifest { items: Vec::new() })
        } else {
